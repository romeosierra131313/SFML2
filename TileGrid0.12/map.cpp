...
void map::addEvent(sf::Vector3i l , int t , int at){
    romeo::revt e;
    e = {.Location = l,
         .type = t,
        ...
        if (e.type == sf::Event::KeyPressed){
            ...
            if (e.key.code == sf::Keyboard::Numpad6){//////
              ...
              if (e.key.code == sf::Keyboard::Z){
                     if(UI->menuselection == 1){ 
                          addEvent(map_map[0].getLocation(),3,0);   } 
                     if(UI->menuselection == 2){ 
                          addEvent(map_map[0].getLocation(),4,0);   }
                     }
          ...
          if (e.type == sf::Event::KeyReleased){
                          ...
                          if (e.key.code == sf::Keyboard::Z){
                             if(UI->menuselection == 0){
                               if(!UI->gethasSelectedEntity()){
                                 for(int i = 0; i<map_entity.size(); i++){    
                                   if(map_entity[i].Am_I_Here(sf::Vector2i(map_map[0].getLocation().x,map_map[0].getLocation().y))){ 
                                        UI->sethasSelectedEntity(true);   
                                        map_entity[i].setID(1000);
                                        UI->setSelected_Entity(&map_entity[i])  ;
                                        UI->setFocus(true);
                                   } 
                                 }
                               }else{
                                   if(UI->getSelected_Entity()->getLocation() == map_map[0].getLocation()){
                                      UI->setFocus(true);
...
//////////////////////////////////DRAW CURSOR////////////////////////////////////
 ...
 //////////////////////////////////DRAW ENTITYS//////////////////////////////////// 
               map_entity[i].handleEvent(UI->event.front());
               
                if(map_entity[i].getID() ==2000){
                     UI->event.pop();
                      UI->menuselection = 0;
                     map_entity[i].setID(1000);
                }
                if(map_entity[i].getID() ==3000){
                    UI->getSelected_Entity()->setID(4000);
                    map_entity[i].setID(0);
                    UI->event.pop();
                    UI->menuselection = 0;
                    UI->sethasSelectedEntity(false);
